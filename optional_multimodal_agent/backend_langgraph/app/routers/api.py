...
@router.post("/api/media/agent-runs", response_model=ApiResponse)
async def create_multimodal_agent_run(
    image: UploadFile = File(...),
    ...
    try:
        request = AgentRunRequest.model_validate(
            {"user_id": user_id, "message": message, "provider": provider}
        )
        analysis = analyze_travel_image(
            image.content_type or "",
            await image.read(),
            "Agent 여행 계획에 필요한 정보와 안전 주의사항을 분석해 주세요.",
        )
        payload = request.model_dump()
        payload["image_analysis"] = analysis.model_dump()
        return ok(start_langgraph_run(payload))
    except ValueError as error:
        raise HTTPException(status_code=422, detail=str(error)) from error
    except Exception as error:
        raise HTTPException(
            status_code=502,
            detail=f"멀티모달 Agent 실행 실패: {error}",
        ) from error
