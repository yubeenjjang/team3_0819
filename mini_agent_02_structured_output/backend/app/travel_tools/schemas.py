...
class TravelPlanParameters(BaseModel):
    """Travel conditions shared by requests, tool calls, and responses."""

    model_config = ConfigDict(extra="forbid")

    city: CityName = Field(description="여행 지역")
    check_in: date = Field(description="여행 시작일(오늘 이후)")
    check_out: date = Field(description="여행 종료일(시작일 이후)")
...
class TravelPlanRequest(TravelPlanParameters):
    ...
    provider: ProviderName | None = Field(
        default=None,
        description="사용할 LLM Provider. 생략하면 서버 기본값을 사용합니다.",
    )
...
class Attraction(BaseModel):
    ...
    description: str = Field(
        min_length=1, max_length=500, description="관광지 추천 설명"
    )
...
class Restaurant(BaseModel):
    ...
    description: str = Field(
        min_length=1, max_length=500, description="맛집 추천 설명"
    )
    latitude: float = Field(ge=-90, le=90, description="위도")
    longitude: float = Field(ge=-180, le=180, description="경도")
    estimated_price_krw: int = Field(
        ge=0, description="1인 기준 예상 가격(원)"
    )
...
class AttractionRecommendationResult(BaseModel):
    ...
    attractions: list[Attraction] = Field(
        min_length=1, max_length=10, description="추천 관광지 목록"
    )
...
class RestaurantRecommendationResult(BaseModel):
    ...
    restaurants: list[Restaurant] = Field(
        min_length=1, max_length=10, description="추천 맛집 목록"
    )
...
class ToolExecutionResult(BaseModel):
    ...
    tool_call_id: str = Field(
        min_length=1, max_length=100, description="원본 Tool 호출 식별자"
    )
    name: TravelToolName = Field(description="실행한 Tool 이름")
    success: bool = Field(description="Tool 실행 성공 여부")
    data: AttractionRecommendationResult | RestaurantRecommendationResult | None = Field(
        default=None, description="정규화된 Tool 실행 결과"
    )
    error: str | None = Field(
        default=None, min_length=1, max_length=500, description="안전한 오류 메시지"
    )
    ...
    def validate_result(self) -> "ToolExecutionResult":
        ...
        if self.name == "recommend_attractions" and self.data is not None:
            if not isinstance(self.data, AttractionRecommendationResult):
                raise ValueError("관광지 Tool 결과 타입이 올바르지 않습니다.")
        if self.name == "recommend_restaurants" and self.data is not None:
            if not isinstance(self.data, RestaurantRecommendationResult):
...
class TravelPlanResponse(BaseModel):
    ...
    tool_calls: list[ToolCall] = Field(
        default_factory=list, max_length=10, description="LLM이 제안한 Tool 호출"
    )
    tool_results: list[ToolExecutionResult] = Field(
        default_factory=list, max_length=10, description="Tool 실행 결과"
    )
