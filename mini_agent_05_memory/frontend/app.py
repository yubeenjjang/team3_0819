...
st.set_page_config(page_title="Mini Agent 05", page_icon="🧠", layout="wide")
...
with st.sidebar:
    ...
    st.divider()
    with st.expander("05. Memory", expanded=True):
        st.page_link(memory_types, label="5-1. Memory 종류")
        st.page_link(conversation, label="5-2. 대화 Window")
        st.page_link(memory_isolation, label="5-3. 사용자별 대화 분리")
